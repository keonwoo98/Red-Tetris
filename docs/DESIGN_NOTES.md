...
| 14 | `game:over` | S→C | `GameOverPayload` `{winnerId,winnerName,reason}` | — | Server's `checkWinner()` found `alivePlayers.size<=1` (multi) or the solo player ended. Sets `status='finished'`, freezes round, promotes the winner to host and re-enables their relaunch. |
...
```
   ...
   └──────┘ ◄─────────────── └─────────┘ ◄──────── winner restart() ──────── └───────┘
       ...
       │                                                            │
       └──────────────── winner restart() also from ended ───────────┘
```
